...
import { buscarProdutos, criarProduto, atualizarProduto, excluirProduto } from '../services/apiService'; // Alterado para apiService e nomes traduzidos
...
const ProductManagementPage: React.FC = () => {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const data = await buscarProdutos(); // Busca todos, incluindo inativos
      setProducts(data);
      setError(null);
    } catch (err: any) {
      console.error("Falha ao buscar produtos:", err);
      setError(err.message || "Não foi possível carregar os produtos para gerenciamento.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveProduct = async (productData: Omit<Product, 'id' | 'imagemUrl'> | Product) => {
    ...
    try {
        if ('id' in productData && productData.id) { // Editando
            await atualizarProduto(productData.id, productData);
        } else { // Criando
            await criarProduto(productData as Omit<Product, 'id' | 'imagemUrl'>);
        }
        setShowForm(false);
