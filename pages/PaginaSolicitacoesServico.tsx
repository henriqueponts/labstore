...
import { 
    ...
    buscarOrcamento,
    criarOrcamentoParaSolicitacao,
    aprovarOrcamento,
    recusarOrcamento,
    concluirSolicitacaoServicoTecnico
} from '../services/apiService'; // Alterado para apiService e nomes traduzidos
...
const ServiceRequestsPage: React.FC = () => {
  ...
  const loadRequests = async () => {
    ...
    try {
      setLoading(true);
      const data = await buscarSolicitacoesServico(); // Passa user.role se a API precisar
      setRequests(data);
      setError(null);
    } catch (err: any) {
  ...
  const getProgress = (status: ServiceRequestStatus): number => {
    switch (status) {
      ...
      case ServiceRequestStatus.CANCELADO: return 0;
      default: return 0;
    }
  };
  ...
  const handleClientSubmitRequest = async (e: FormEvent) => {
    ...
    try {
      await criarSolicitacaoServico(newRequestData); // userId é inferido pelo backend
  ...
  const handleViewQuote = async (request: ServiceRequest) => {
    ...
    try {
        const quote = await buscarOrcamento(request.quoteId);
  ...
  const handleApproveQuote = async () => {
    ...
    try {
        await aprovarOrcamento(selectedRequestForQuoteView.id, viewingQuote.id);
        setViewingQuote(null);
        setSelectedRequestForQuoteView(null);
        await loadRequests();
        alert("Orçamento aprovado! O serviço será iniciado.");
    } catch (err: any) {
        setError(err.message || "Falha ao aprovar orçamento.");
    }
  };
  ...
  const handleRefuseQuote = async () => {
    if (!user || !selectedRequestForQuoteView || !viewingQuote ) return;
    try {
        await recusarOrcamento(selectedRequestForQuoteView.id, viewingQuote.id, motivoRecusa || "Recusado pelo cliente");
        setViewingQuote(null);
        setSelectedRequestForQuoteView(null);
        setMotivoRecusa('');
        await loadRequests();
        ...
        setError(err.message || "Falha ao recusar orçamento.");
    }
  };
  ...
  const handleTechSubmitQuote = async (e: FormEvent) => {
    ...
    try {
        await criarOrcamentoParaSolicitacao(selectedRequestForTechQuote.id, newQuoteData); // analistaId inferido no backend
        ...
        setError(err.message || "Falha ao criar orçamento.");
    } finally {
  ...
  const handleCompleteService = async (requestId: string) => {
    if(!user) return;
    try {
        await concluirSolicitacaoServicoTecnico(requestId); // technicianId inferido no backend
        await loadRequests();
