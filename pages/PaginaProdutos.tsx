...
import { buscarProdutos, adicionarAoCarrinho } from '../services/apiService'; // Alterado para apiService e nomes traduzidos
...
const ProductsPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const data = await buscarProdutos(); // Chama a função traduzida
  ...
  const handleAddToCart = async (product: Product) => {
    ...
    try {
        await adicionarAoCarrinho(product.id, 1); // Chama a função traduzida
        alert(`${product.nome} adicionado ao carrinho!`);
    } catch (err: any) {
        alert(err.message || `Erro ao adicionar ${product.nome} ao carrinho.`);
        console.error(err);
    }
  };
