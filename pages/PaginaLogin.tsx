...
const LoginPage: React.FC = () => {
  ...
  const { login, isLoading } = useAuth(); // A função 'login' do contexto já usa 'entrar' do apiService
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    try {
      const user = await login(email, password); // Chama context.login
      if (user) {
      ...
      setError(err.message || 'Falha ao tentar fazer login. Tente novamente mais tarde.');
      console.error(err);
    }
  };
