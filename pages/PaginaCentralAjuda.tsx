...
import { buscarChamadosAjuda, criarChamadoAjuda } from '../services/apiService'; // Alterado para apiService
...
const HelpDeskPage: React.FC = () => {
  ...
  const [newTicket, setNewTicket] = useState<Omit<HelpTicket, 'id' | 'clienteId' | 'data_abertura' | 'status' | 'respostas'>>({
    assunto: '',
    descricao: '',
    categoria: 'Problemas com a entrega',
  });
  ...
  const loadTickets = async () => {
    ...
    try {
      setLoading(true);
      const data = await buscarChamadosAjuda(); // Passa user.role se a API precisar
      setTickets(data);
      setError(null);
    } catch (err: any) {
  ...
  const handleSubmitTicket = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
        await criarChamadoAjuda(newTicket); // clienteId é inferido no backend
        setShowForm(false);
        ...
        setError(err.message || "Falha ao abrir chamado. Tente novamente.");
    } finally {
