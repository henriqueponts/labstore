...
const OrdersPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadOrders = async () => {
      ...
      try {
        setLoading(true);
        const data = await fetchOrders(user.id); // Busca pedidos para o usuário atual
        setOrders(data);
        setError(null);
      } catch (err) {
        ...
        <div className="space-y-6">
          {orders.map(order => (
            <Card key={order.id} title={`Pedido #${order.id.substring(0,8)}... - ${new Date(order.data_pedido).toLocaleDateString()}`}>
              ...
              <div className="mt-4">
                <Button variant="ghost" size="sm">Ver Detalhes do Pedido</Button> {/* TODO: Link para página de detalhes do pedido */}
              </div>
            </Card>
          ))}
        </div>
